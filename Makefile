SCHEME      = Clipper
DERIVED     = $(HOME)/Library/Caches/ClipperBuild
APP         = $(DERIVED)/Build/Products/Debug/Clipper.app
XCB         = xcodebuild -project Clipper.xcodeproj -scheme $(SCHEME) -derivedDataPath $(DERIVED) -destination 'platform=macOS' CODE_SIGN_IDENTITY=- CODE_SIGNING_ALLOWED=YES
...
.PHONY: gen build sign run stop test install clean
...
stop:
	-pkill -x Clipper 2>/dev/null || true

test: gen $(DERIVED)
	$(XCB) test 2>&1 | tee $(DERIVED)/test.log | grep -E "error:|Test Case.*(passed|failed)|Executed|BUILD|TEST" || true

install: sign stop
	rm -rf /Applications/Clipper.app
	cp -R $(APP) /Applications/Clipper.app
	open /Applications/Clipper.app

clean:
	rm -rf $(DERIVED) Clipper.xcodeproj
